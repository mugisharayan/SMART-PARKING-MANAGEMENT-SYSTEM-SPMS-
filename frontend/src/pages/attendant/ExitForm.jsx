            ...
            <div style={{ maxWidth: 480, margin: '0 auto' }}>
              ...
              {session && !done && (
                <div style={{ background: 'var(--gray-50)', border: '1px solid var(--gray-200)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-4)', marginBottom: 'var(--space-4)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 'var(--space-4)', marginBottom: 'var(--space-3)' }}>
                    {[['Plate', session.plateNumber, true, 'var(--gray-900)'], ['Slot', session.slotId, true, 'var(--brand-primary)'], ['Duration', elapsed(session.entryTime), false, 'var(--color-occupied)']].map(([k, v, mono, color]) => (
                      <div key={k} style={{ textAlign: 'center' }}>
                        ...
                        <div style={{ fontFamily: mono ? 'var(--font-mono)' : undefined, fontWeight: 800, fontSize: 'var(--text-lg)', color }}>
                          {v}
                          {k === 'Duration' && elapsedHours(session.entryTime) >= LONG_STAY_HOURS && (
                            <span className="badge badge-warning" style={{ fontSize: '0.6rem', marginLeft: 6 }}>Long stay</span>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              {notFound && !done && (
                <div style={{ marginBottom: 'var(--space-4)' }}>
                  <div className="alert alert-danger" style={{ marginBottom: 'var(--space-3)' }}>
                    ...
                    No active session found for <strong>{plate}</strong>.
                  </div>
                  <button className="btn btn-outline-gray btn-sm" onClick={() => setForceConfirm(true)} style={{ width: '100%' }}>
              ...
              {done && (
                <div className="alert alert-success" style={{ marginBottom: 'var(--space-4)' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink: 0 }}><polyline points="20 6 9 17 4 12"/></svg>
                  {closedSlot === '—'
